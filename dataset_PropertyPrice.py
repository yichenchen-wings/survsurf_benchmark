...
class DatasetHousePrice(Dataset):
    def __init__(
        ...
        self.colname_traj_id = 'traj_id'
        self.colname_time = 't'
        self.colname_g = 'g_max_by_time'
        self.g_max = g_max
        n_steps = n_steps
        if equal_steps:
            self.gs_grid = np.array([self.g_max*(i+1)/n_steps for i in range(n_steps)])
        else:
            self.gs_grid = np.array([self.g_max/(i+1) for i in range(n_steps)])
        self.ts_grid = np.array([(i+1) for i in range(7)])
...
class DataModuleHousePrice(LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        as_true_prob = DatasetHousePrice(
            self.df_dir, 
            self.ds_name, 
            self.g_resol, 
            split='val', 
